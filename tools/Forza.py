...
class Forza:
        ...
        try:
            ...
            with open(self.configFile, 'r') as file:
                for line in file:
                    ...
                    print(
                        f"Loaded field: {name} with format {fmt}, size {size}, offset {offset}")
                    offset += size
    ...
    def get_value(self, data, name):
        ...
        if len(data) < offset + size:
            print(
                f"Data too short for field {name}. Expected at least {offset + size} bytes, got {len(data)} bytes.")
            return None
    ...
    def get_offset(self, name):
        if name.lower() not in self.config:
            print(f"Field {name} not found in config.")
            return None
        return self.config[name.lower()]['offset']
