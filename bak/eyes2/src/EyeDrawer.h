...
class EyeDrawer {
   public:
    ...
    static void FillEllipseCorner(CornerType corner, int16_t x0, int16_t y0, int32_t rx, int32_t ry, uint16_t color) {
        ...
        if (corner == T_R) {
            for (x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry); ry2 * x <= rx2 * y; x++) {
                sprite.drawFastHLine(x0, y0 - y, x, mapU8g2ColorToTFT(color));
                if (s >= 0) {
                    s += fx2 * (1 - y);
                    y--;
                }
            ...
            for (x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry); ry2 * x <= rx2 * y; x++) {
                sprite.drawFastHLine(x0, y0 + y - 1, x, mapU8g2ColorToTFT(color));
                if (s >= 0) {
                    s += fx2 * (1 - y);
                    y--;
                }
            ...
            for (x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry); ry2 * x <= rx2 * y; x++) {
                sprite.drawFastHLine(x0 - x, y0 - y, x, mapU8g2ColorToTFT(color));
                if (s >= 0) {
                    s += fx2 * (1 - y);
                    y--;
                }
            ...
            for (x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry); ry2 * x <= rx2 * y; x++) {
                sprite.drawFastHLine(x0 - x, y0 + y - 1, x, mapU8g2ColorToTFT(color));
                if (s >= 0) {
                    s += fx2 * (1 - y);
                    y--;
                }
    ...
    static void FillRectangle(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t color) {
        ...
        sprite.fillRect(l, t, w, h, mapU8g2ColorToTFT(color));
    ...
    static void FillRectangularTriangle(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t color) {
        // u8g2.setDrawColor(color);
        sprite.fillTriangle(x0, y0, x1, y1, x1, y0, mapU8g2ColorToTFT(color));
        // u8g2.setDrawColor(1);
    }

    static void FillTriangle(int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t color) {
        // u8g2.setDrawColor(color);
        sprite.fillTriangle(x0, y0, x1, y1, x2, y2, mapU8g2ColorToTFT(color));
        // u8g2.setDrawColor(1);
    }
};
